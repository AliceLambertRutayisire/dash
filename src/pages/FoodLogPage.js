...
const TABLE_HEAD = [
  ...
  { id: 'water/beverage', label: 'Water/beverage', alignRight: false },
  { id: '' },
];
...
export default function FoodLogPage() {
  ...
  return (
    <>
      <Helmet>
        <title> Food log | Minimal UI </title>
      </Helmet>
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { id, breakfast, lunch, dinner, snacks, water } = row;
                    const selectedUser = selected.indexOf(breakfast) !== -1;

                    return (
                      <TableRow hover key={id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        <TableCell padding="checkbox">
                          <Checkbox checked={selectedUser} onChange={(event) => handleClick(event, breakfast)} />
                        </TableCell>
